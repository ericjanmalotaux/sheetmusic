...
In bars 73-83 of the superius, <<BrusBR9126>> does not have the stem:[3/1] indication that the other voices have, but it is implied, because otherwise the voice would not fit to the other ones. The notes are written black, instead of open, the meaning of which is not clear to me. The same for bars 84-86.
...
* get the alla-breve symbol [big]*&#119093;*, which Lilypond by default uses only for stem:[2/2] meter, while _Gaude Virgo_ has 2/1, but indicates alle-breve in the source
* have different meters, stem:[2/1] and stem:[3/1] in different voices at the same time
...
* correctly scale the note durations for the stem:[3/1] sections, so as to properly align them to the other voices that are still in stem:[2/1] meter
* obtain the correct tempo ratio for the stem:[3/1] sections in the MIDI output
